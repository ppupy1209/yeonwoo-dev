import { ArrowDefs, Arrow, Line, NodeBox, CircuitBreakerIcon } from "./primitives";
...
export function PhaseAutoCloseArch() {
  return (
    ...
    >
      ...
      <NodeBox x={10} y={14} w={155} h={52} label="인스턴스 A" sub="스케줄러 실행 중" logo={SPRING} />
      <NodeBox x={10} y={78} w={155} h={52} label="인스턴스 B" sub="스케줄러 실행 중" logo={SPRING} />
      <NodeBox x={10} y={142} w={155} h={52} label="인스턴스 C" sub="스케줄러 실행 중" logo={SPRING} />
      ...
      <NodeBox x={199} y={78} w={155} h={52} label="Redis 분산락" logo={REDIS} highlight />
      ...
      <NodeBox x={559} y={78} w={153} h={52} label="종료 로직 수행" sub="국면 상태 변경" icon={CircuitBreakerIcon} />
    </svg>
  );
}
