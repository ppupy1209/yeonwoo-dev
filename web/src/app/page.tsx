...
export default function Home() {
  ...
  return (
    <>
      ...
      <section id="work" className="scroll-mt-24 mx-auto max-w-3xl px-6 pb-8">
      ...
      <section id="studies" className="scroll-mt-24 mx-auto max-w-3xl px-6 pb-16 pt-8">
        <div className="flex items-center gap-2.5">
          <span className="h-5 w-1 rounded-full bg-accent" aria-hidden />
          <h2 className="text-xl font-semibold">Studies</h2>
        </div>
        <p className="mt-1.5 text-sm text-muted">
          대규모 트래픽·분산 환경을 직접 구현하며 검증한 학습 프로젝트
        </p>
        <div className="mt-6 grid gap-4">
          {study.map((project) => (
            <ProjectCard key={project.slug} project={project} />
