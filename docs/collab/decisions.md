...
## ADR-012 · 2026-06-11 · accepted
**계약 명료화(백엔드 M-001 제안 수락): 방명록 POST honeypot 응답은 `200 {ok:true}`(연락 API와 동일), `size` 범위 초과는 `400 validation`으로 거부(클램프 안 함).**
백엔드 구현·제안을 계약에 명시해 프론트 처리를 단순화.
